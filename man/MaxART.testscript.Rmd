...
testinput$person.eagerness.dist.type <- "gamma"
testinput$person.eagerness.dist.gamma.a <- 0.85#1
testinput$person.eagerness.dist.gamma.b <- 20#16#3
...
testinput$person.agegap.woman.dist.normal.mu <- -5
...
testinput$formation.hazard.agegapry.gap_factor_man_exp <- -0.1# -0.5
testinput$formation.hazard.agegapry.gap_factor_woman_exp <- -0.1# -0.5
testinput$formation.hazard.agegapry.gap_factor_man_const <- 0
testinput$formation.hazard.agegapry.gap_factor_woman_const <- 0
testinput$formation.hazard.agegapry.gap_agescale_man <- 0.3
testinput$formation.hazard.agegapry.gap_agescale_woman <- 0.3
testinput$formation.hazard.agegapry.eagerness_sum <- 0.1
testinput$formation.hazard.agegapry.eagerness_diff <- -0.0975
testinput$dissolution.alpha_0 <- -0.1 # baseline
testinput$dissolution.alpha_4 <- -0.05 
testinput$debut.debutage <- 14
testinput$population.simtime <- 80
testinput$population.nummen <- 1000
testinput$population.numwomen <- 1000
...
testplot.prev <- prevalence.plot(prev.agegroup)
testplot.prev
agemix.df <- agemix.df.maker(datalist.test)
agemix.pattern <- pattern.modeller(dataframe = agemix.df, agegroup = c(15, 60), timepoint = 20, timewindow = 1, start = FALSE)
testplot.agemix <- pattern.plotter(agemix.pattern)
testplot.agemix
incidence.calculator(datalist = datalist.test, agegroup = c(15,20), timewindow = c(40, 50))

degree.df <- degree.df.maker(agemix.df, survey.time = 40, window.width = 1, only.new = TRUE)
degree.hist <- degree.hist.maker(dataframe = degree.df)
degree.hist
gamma.dens <- dgamma(seq(0.1, 20, by = 0.1), shape = 0.95,#datalist.test$itable$person.eagerness.dist.gamma.a,
       scale = 5)#datalist.test$itable$person.eagerness.dist.gamma.b)
plot(seq(0.1, 20, by = 0.1), gamma.dens, type = "l")
