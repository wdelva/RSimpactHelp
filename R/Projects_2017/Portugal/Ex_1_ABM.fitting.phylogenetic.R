...
rm(list = ls())
setwd("/home/david/Dropbox/Fitting_Simpact/FitEpiPhylo/EX_1/")
...
if(comp == "win"){dirname <- "~/MaxART/RSimpactHelp"}else if(comp=="lin"){
  dirname <- "~/Dropbox/Fitting_Simpact/FitEpiPhylo/EX_1/"}else if(comp=="chpc"){
...
inPUT.df.complete <- simpact.config.inputs(design.points = init.design.points,
                                           resample.count = 2,
#
...
target.variables <- c("rels.rate", "R0")
...
filename.run <- paste0(dirname,"/","SummaryOutPut-df-phylo-exp_1-",min.chunk,"-",max.chunk,".csv")
