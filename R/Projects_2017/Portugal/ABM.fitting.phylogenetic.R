...
if(comp == "win"){dirname <- "~/MaxART/RSimpactHelp"}else if(comp=="lin"){
  dirname <- "~/Dropbox/Fitting_Simpact/FitEpiPhylo/"}else if(comp=="chpc"){
...
simpact4ABC.chunk.wrapper <- function(simpact.chunk.prior){
  ...
  simpact.chunk.run <- function(input.chunk.params){
    ...
    if(length(chunk.datalist.test)>1){
      ...
      hiv.seq.env <- substr(clean.hiv.seq, 6172,6180) # true c(6172,8742)
...
filename.run <- paste0(dirname,"/","SummaryOutPut-df-phylo-fit-",min.chunk,"-",max.chunk,".csv")
...
###################################################################################
rm(list = ls())
setwd("/home/david/Dropbox/Fitting_Simpact/FitEpi/")
...
if(comp == "win"){dirname <- "~/MaxART/RSimpactHelp"}else if(comp=="lin"){
  dirname <- "~/Dropbox/Fitting_Simpact/FitEpi/"}else if(comp=="chpc"){
    dirname <- "/mnt/lustre/users/tchibawara/MaxART/data"}else if(comp=="gent"){
      dirname <- "/user/data/gent/vsc400/vsc40070/simpact-test/data"}else{
        dirname <- "~/Documents/RSimpactHelp"  #mac directory here
      }
...
simpact4ABC.chunk.wrapper <- function(simpact.chunk.prior){
  ...
  simpact.chunk.run <- function(input.chunk.params){
    ...
    if(length(chunk.datalist.test)>1){
      ...
      out.statistics <- c(growth.rate, rels.rate, transm.rate,  R0)
...
filename.run <- paste0(dirname,"/","SummaryOutPut-df-fit-",min.chunk,"-",max.chunk,".csv")
