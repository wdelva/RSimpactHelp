...
transNetworkNew <- function(datalist = datalist, endpoint = 40){
    ...
    for(i in 1:length(seeds.id)){
        ...
        vl.ids <- subset(vltable, vltable$ID%in%ids)
        
        id.vl <- vector("list", length(ids))
        for(j in 1:length(ids)){
            id.vl[[j]] <- subset(vl.ids,vl.ids$ID==ids[j])
        }
        
        ids.samplingtime <- (b[[i]][,1]) # Bear in mind to change log scale and timing
        ids.infecttime <- (dat.recdontime[[i]][,5])
        
        # Now place holder: calculate using model
        ids.time <- ((b[[i]][,1]) - (dat.recdontime[[i]][,5]))
        N0 = 10
    ...
    transNet <- list()
    for(i in 1:length(seeds.id)){
        transNet$itimes <- (dat.recdontime[[i]][,5])
        transNet$dtimes <- (b[[i]][,1])
        transNet$id <- dat.recdontime[[i]][,2] # 1
        transNet$parent <- dat.recdontime[[i]][,3] # 4
        transNet$vload <- round(Nt[[i]])
