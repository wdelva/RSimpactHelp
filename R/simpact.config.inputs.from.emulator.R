...
#'
#' @param datalist the selected min parameters that produced minimal sum os squares
#'
...
simpact.config.inputs.from.emulator <- function(datalist, ...){
  ...
  set.seed(1)
  rlhs <- dplyr::select(datalist, contains("xdesign"))
  ...
  lhs.df <- as.data.frame(lapply(lhs.df, rep, nrow(datalist)))
  ...
  simpactInputParams <- cbind.data.frame(sim.id = 1:nrow(datalist), rlhs, lhs.df)

  write.csv(simpactInputParams, file =paste0("simpactInputParams.df.EmuBest","-",nrow(datalist),"Points",variables,"Par",Sys.Date(), ".csv"), row.names = FALSE)
