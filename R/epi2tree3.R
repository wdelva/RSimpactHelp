...
epi2tree3 <- function(epi){
  make.internal <- function(epi, node, parent, cur.id) {
    ...
    if (sum(offspring) > 0) {
     ...
     # intervals <- abs( c(epi$itimes[node.i], inf.t) - c(inf.t, dt))
...
#         if(length(aa)>2){
#
#           aa[length(aa)-1]  <- inf.t[length(inf.t)-1] - dt
#           aa[length(aa)] <- dt - inf.t[length(inf.t)]
#           intervals <- aa
#
#         }
#         if(length(aa)==2){
#
#           aa[length(aa)-1]  <- epi$itimes[node.i] - dt
#           aa[length(aa)] <- dt - inf.t[length(inf.t)]
#           intervals <- aa
#
#         }
...
#       }
      ...
      aa <- c(epi$itimes[node.i], inf.t) - c(inf.t, dt)
      ...
      if(aa[length(aa)]<0){
        ...
        # aa[length(aa)-1]
        # dt-aa[length(aa)]
        # dt-aa[length(aa)-1] + aa[length(aa)]
        # dt-aa[length(aa)-1] - aa[length(aa)]
        # dt-aa[length(aa)-1] + dt-aa[length(aa)]
        # aa[length(aa)-1]
        # dt-aa[length(aa)-1]
        intervals <- aa
      }else{
