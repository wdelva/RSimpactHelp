...
hiv.infected.group <- function(datalist = datalist, agegroup = c(15, 30), timewindow = c(20, 30), site="All"){
  ...
  #exlude TOD but this should be excluded already
  person.alive.infected <- subset(person.alive.infected, InfectTime>exposure.start & InfectTime<=exposure.end)
  #aggregate by gender
