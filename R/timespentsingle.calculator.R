...
#' @param type If type is "Strict", all time spent being not being in any relationship will be excluded from exposure time.
#' If type is "Harling", time will be excluded from exposure time in blocks of one year, if the person spent that entire block not in any relationship.
...
timespentsingle.calculator <- function(datalist = datalist,
  ...
  # Now we augment the women.no.rels dataset with the exposure.start and exposure.end data from the women.filtered.df dataset
  ...
  women.no.rels.plus$norels.start.real <- pmax(women.no.rels.plus$norels.start, women.no.rels.plus$exposure.start)
  women.no.rels.plus$norels.end.real <- pmin(women.no.rels.plus$norels.end, women.no.rels.plus$exposure.end)

  norels.time <- women.no.rels.plus$norels.end.real - women.no.rels.plus$norels.start.real # This is the naive time period spent single, before tidying up
  really.norels.time <- norels.time > 0 # We create a vector to see which episodes were REALLY spent single during the exposure time window
  women.no.rels.plus.clean <- women.no.rels.plus[really.norels.time, ]
  women.no.rels.plus.clean$norels.timespent <- women.no.rels.plus.clean$norels.end.real - women.no.rels.plus.clean$norels.start.real

  # For the Harling version
  fract.survey <- women.no.rels.plus.clean$exposure.start %% 1 # How far in the calendar year did the interview with the woman take place (baseline at time of exposure.start, and then each time 1 year thereafter)
  fract.norels.start.real <- women.no.rels.plus.clean$norels.start.real %% 1 # How far in the calendar year did the period of being single start?
  move.to.last.survey <- ceiling(fract.survey - fract.norels.start.real)
  women.no.rels.plus.clean$norels.start.surveytime <- ceiling(women.no.rels.plus.clean$norels.start.real) + fract.survey - move.to.last.survey
  ...
  if (type == "Strict"){
    ...
    norels.timespent.df <- dplyr::summarise(group_by(women.no.rels.plus.clean, woman.ID),
