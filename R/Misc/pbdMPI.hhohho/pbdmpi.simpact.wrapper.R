#loading packages
#torun local use
# mpirun -np 4 Rscript R/Misc/pbdMPI.hhohho/pbdmpi.simpact.wrapper.R
#pacman::p_unload(all)
...
if(sim.initiate == TRUE){
  ...
  dp.val <- 8 #create parameter space
  ...
  mins.val <- 5 #select from the parameter space min
  maxs.val <- 8 #select from teh parameter space max
