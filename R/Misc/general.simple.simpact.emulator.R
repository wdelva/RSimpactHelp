...
simpact.inANDout.df <- read.csv(file = file.name.csv, header = TRUE, sep = ",")
...
# growth.rate
# prev.15.50.end
z.variable = c("growth.rate", "prev.15.50.end")
## Defining the target statistics z.variables for which we will seek the most likely parameters.
targets <- c(0, 0.1)
# z1 (growth.rate) ~ 0
# z2 (prev.15.50.end) ~ 0.1

#select the x model param values
simpact.x <- dplyr::select_(simpact.inANDout.df,.dots=x.variables) %>% as.matrix()
#select the z model param values
simpact.z <- dplyr::select_(simpact.inANDout.df,.dots=z.variable)

sum(duplicated(simpact.x))
sum(duplicated(simpact.z))
...
x1.design <- punif(simpact.x[, 1],min = round(min(simpact.x[, 1])), max = round(max(simpact.x[, 1])))
x.design <- data.frame(x1.design)

#### Setting up design. Creating the LHS over the 0-1 uniform parameter space
design.points <- nrow(x.design) #40 #20
variables <- length(x.variables)

for( i in 2:variables) {
  x.design <- cbind(x.design, punif(simpact.x[, i],min = round(min(simpact.x[, i])), max = round(max(simpact.x[, i]))))
...
x.design.long <- x.design[rep(1:nrow(x.design),length(z.variable)),]
...
RS_new_mdm <- mdm(rbind(x.new, x.new), types = rep(z.variable, each = n))
...
prediction.df <- data.frame(matrix(RS_prediction_c, nrow = n, dimnames = list(rownames = 1:n, colnames = z.variable)))
...
sq.array <- as.data.frame(t(apply(prediction.df, 1, function(x) (x - t(targets))^2)))
