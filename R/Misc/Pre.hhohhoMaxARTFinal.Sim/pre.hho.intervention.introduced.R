...
intervention.introduced <- function(simulation.type = "pre.hhohho", simulation.start = "1970-03-31"){
  ...
  if(simulation.type == "standard.care"){
    ...
    art.intro["monitoring.cd4.threshold.instudy.controlstage"] <- 200
    art.intro["monitoring.cd4.threshold.instudy.transitionstage"] <- 200
    art.intro["monitoring.cd4.threshold.instudy.interventionstage"] <- 200
    ...
    art.intro2["monitoring.cd4.threshold.instudy.controlstage"] <- 200
    art.intro2["monitoring.cd4.threshold.instudy.transitionstage"] <- 200
    art.intro2["monitoring.cd4.threshold.instudy.interventionstage"] <- 200
    ...
    art.intro3["monitoring.cd4.threshold.instudy.transitionstage"] <- 350
    art.intro3["monitoring.cd4.threshold.instudy.interventionstage"] <- 350
    ...
    art.intro4["monitoring.cd4.threshold.instudy.transitionstage"] <- 500
    art.intro4["monitoring.cd4.threshold.instudy.interventionstage"] <- 500
    art.intro4["monitoring.interval.piecewise.cd4s"] <- "500,650"
    art.intro4["diagnosis.genderfactor"] <- 0.5

    art.time7 <- as.Date("2016-10-02")
    art.intro7 <- list()
    ...
    art.intro7["monitoring.cd4.threshold.instudy.controlstage"] <- 20000
    art.intro7["monitoring.cd4.threshold.instudy.transitionstage"] <- 20000
    art.intro7["monitoring.cd4.threshold.instudy.interventionstage"] <- 20000
    art.intro7["monitoring.interval.piecewise.cd4s"] <- "500,20000"
    ...
    art.intro7 <- list()
    art.intro7["time"] <- round(as.numeric(difftime(art.time4,start.time.sim, units = "days")/365.242),0)
    art.intro7["monitoring.cd4.threshold.prestudy"] <- 20000
