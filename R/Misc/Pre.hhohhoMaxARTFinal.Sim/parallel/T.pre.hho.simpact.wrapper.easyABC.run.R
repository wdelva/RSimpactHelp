...
pacman::p_load(dplyr, EasyABC, RSimpactCyan, RSimpactHelper, lhs, pbdMPI)
...
sel.id.list <- c(25:49)  # or c(1,3,5)
...
sim.run <- function(inANDout.df.chunk){
  ...
  #Artificate of pbdApply ...
  ABC.results.chunk.statistics <- paste(unlist(ABC.results.chunk.statistics), collapse = "|")

  return(ABC.results.chunk.statistics)

}
#print(dim(inANDout.df.chunk))

test.df <- as.matrix(inANDout.df.chunk)
res <- pbdApply(test.df, 1, sim.run, pbd.mode = "mw")

if(.comm.rank==0){

  res <- as.data.frame(res)

  write.csv(res, file = paste0(dirname,"/testing.me.mpi.OUT.first.csv"), row.names = FALSE)

  chunk.summary.stats.df <- read.table(text = res$res, sep = "|", colClasses = "numeric")
  names(chunk.summary.stats.df) <- c(target.variables,"sim.id")

  write.csv(chunk.summary.stats.df, file = paste0(dirname,"/testing.me.mpi.OUT.after.csv"), row.names = FALSE)
  ...
  filename.run <- paste0(dirname,"/","SummaryOutPut-df-",min.chunk,"-",max.chunk,"TESTpdbMPI.csv")
