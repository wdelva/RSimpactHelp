...
inPUT.df.complete <- source.simpact.parameters(init.design.points = 24, resample = 1)

#select a chunk to process
sel.id.list <- c(1:24)  # or c(1,3,5)
...
sim.run <- function(inANDout.df.chunk){
  #turn matrix row to a named data.frame
  ...
  ABC.results.chunk.statistics <- data.frame(ABC.chunk.result$stats, chunk.sim.id)

  #turn the result to a single value (string) which will be unpacked later
  #artificate of pbdApply (need to retun a single value)
...
#write.csv(res, file = paste0(dirname,"/testing.me.mpi.csv"), row.names = FALSE)
