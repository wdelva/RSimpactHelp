#!/usr/bin/env/ Rscript
foreach.simulation.wrapper <- function(set.seed.id = 1,   #what seed to use
                                       design.points = 100,
                                       par.repeat = 4,    #each row is repeated once.
  ...
  cl <- makeCluster(ncluster.use)
  clusterSetRNGStream(cl, set.seed.id)
  registerDoParallel(cl)

  chunk.summary.stats.df <-
    foreach(row.df.input = iter(inANDout.df.chunk, by = 'row'),
            .combine = rbind) %dopar%
            {
              #simpact wrapper giving simulated target statistics
              sim.run(row.df.input)
            }
  stopCluster(cl)
  ...
  filename.run <- paste0(dirname,"/", save.name, min.sim,"-", max.sim,
                         "-SummaryOutPut-",rand.string,"-foreach.csv")
