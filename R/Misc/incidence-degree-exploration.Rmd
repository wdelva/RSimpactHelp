...
simpact.set.simulation("maxart")#("simpact-cyan")#
...
testinput$person.eagerness.man.dist.type <- "gamma"
testinput$person.eagerness.woman.dist.type <- "gamma"
testinput$person.eagerness.man.dist.gamma.a <- 0.231989836885#0.15 #0.425#3.4#1.7#0.85 #0.1
testinput$person.eagerness.man.dist.gamma.b <- 45#70#100 #3.5#5#10#20 #170
testinput$person.eagerness.woman.dist.gamma.a <- 0.231989836885#0.15 #0.425#3.4#1.7#0.85 #0.1
testinput$person.eagerness.woman.dist.gamma.b <- 45#70#100 #3.5#5#10#20 #170
...
testinput$hivtransmission.param.a <- -1.0352239
testinput$hivtransmission.param.b <- -89.339994
testinput$hivtransmission.param.c <- 0.4948478
testinput$hivtransmission.param.f1 <- log(5) # ~1.6 such that the hazard is x 5 in 15 yo
testinput$hivtransmission.param.f2 <- log(log(2.5) / log(5)) / 5
...
cfg <- list()
testoutput <- simpact.run(configParams = cfg,
                          destDir = "temp",
...
test.output <- function(testoutput){
  ...
  #degree.vector <- degree.df$Degree # Storing the number of relationships in the last year, for each person in the imaginative survey
  
  output.list <- list(summStats = summStats, degree.dataframe = degree.df)
...
inANDout.10.df <- cbind.data.frame(sim.id = 1:design.points,
                                rlhs,
                                lhs.df,
...
degree.dist.summStats.df <- data.frame(sim.id = 1:design.points,
                                       mean.degree = rep(NA, design.points),
                                       median.degree = rep(NA, design.points),
                                       Q1.degree = rep(NA, design.points),
                                       Q3.degree = rep(NA, design.points))

# And a list of best fitting functions to the degree data
bestfit.list <- vector("list", design.points)  
```
...
simpact4emulation <- function(sim.id, lhs.df, testinput, agedist.data.frame, iv){
  ...
  testinput$person.eagerness.man.dist.gamma.a <- lhs.df$eagerness.a[sim.id] #1
  testinput$person.eagerness.man.dist.gamma.b <- lhs.df$eagerness.b[sim.id] #2
  testinput$person.eagerness.woman.dist.gamma.a <- lhs.df$eagerness.a[sim.id] #1
  testinput$person.eagerness.woman.dist.gamma.b <- lhs.df$eagerness.b[sim.id] #2
  ...
  output.list <- test.output(testoutput)
...
for (sim.id in inANDout.10.df$sim.id){
  #out.test <- simpact4emulation(sim.id, lhs.df, testinput, agedist.data.frame, iv)
  out.test.list <- tryCatch(simpact4emulation(sim.id, lhs.df, testinput, agedist.data.frame, iv), error = errFunction)
  # Inserting the output to the inANDout dataframe
  inANDout.10.df[sim.id, 22:30] <- as.numeric(out.test.list$summStats)
  
  # Let's create a dataframe where we store summary statistics of the degree distribution
  # A first obvious summary statistic is the mean of the degree distribution
  if (nrow(out.test.list$degree.dataframe) >=10 ){
    degree.vector <- out.test.list$degree.dataframe$Degree
    ...
    degree.best.fit <- degree.distr.fitter(out.test.list$degree.dataframe)
...
```{r plotting.degree.dist.fits, echo=TRUE, message = FALSE, cache = FALSE}
bestfit.list.trimmed <- Filter(Negate(is.null), bestfit.list)
bestfit.distname <- unlist(lapply(bestfit.list.trimmed, function(x) x[["distname"]]))
bestfit.list.trimmed <- bestfit.list.trimmed[bestfit.distname=="lnorm"]
bestfit.params <- unlist(lapply(bestfit.list.trimmed, function(x) x[["estimate"]]))

lognormal.estimates <- matrix(bestfit.params,
       byrow = TRUE,
       ncol = 2,
       dimnames = list(rownames = 1:length(bestfit.list.trimmed), colnames = c("meanlog", "sdlog")))

plot(lognormal.estimates[, 1], lognormal.estimates[, 2])
xvector <- 1:10
predictions <- data.frame(t(apply(lognormal.estimates, 1, function(x) dlnorm(xvector, meanlog = x[1], sdlog = x[2], log = FALSE))))
model.data <- cbind.data.frame(lognormal.estimates, predictions)
model.data$sim.id <- 1:nrow(model.data)
model.data.long <- tidyr::gather(data = model.data,
              key = "degree.label",
              value = "density",
              X1:X10)
model.data.long$degree <- tidyr::extract_numeric(model.data.long$degree.label)
line.plot<- ggplot(data = model.data.long, mapping = aes(x = degree, y = density, group=sim.id))
line.plot + geom_line()
