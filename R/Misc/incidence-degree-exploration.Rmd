...
```{r testrun, echo=FALSE, message = FALSE, cache = FALSE}
...
test.output <- function(testoutput){
  ...
  summStats <- matrix(c(growth.rate, median.AD, Q1.AD, Q3.AD, prev.men.15.25, prev.men.25.50, ART.cov.15.50, incid.wom.15.30, frac.degreeGT1.wom.15.30),
                      nrow = 1, dimnames = list(NULL, c("growth.rate", "median.AD", "Q1.AD", "Q3.AD", "prev.men.15.25", "prev.men.25.50", "ART.cov.15.50", "incid.wom.15.30", "frac.degreeGT1.wom.15.30"))) # 8 summary statistics
  return(summStats)
}
```
...
lhs.df$eagerness.a <- qunif(rlhs[ , 1], min = 0.1, max = 2) #max = 1)
lhs.df$eagerness.b <- qunif(rlhs[ , 2], min = 5, max = 60) #min = 25, max = 90) #10, max = 100)
lhs.df$concept.base <- qunif(rlhs[ , 3], min = -3.6, max = -1.2) #min = -2.6, max = -1.2) #-4, max = -1)
lhs.df$numrel <- qunif(rlhs[ , 4], min = -1.5, max = -0.1) #max = -0.2) #-1, max = -0.1)
lhs.df$eagerness.diff <- qunif(rlhs[ , 5], min = -0.1, max = 0) #min = -0.25, max = -0.04) #-0.2, max = -0.02)
lhs.df$gapfactor.exp <- qunif(rlhs[ , 6], min = -1.5, max = -0.4) #-1, max = -0.1)

# Creating a dataframe for input AND output
inANDout.6000.df <- cbind.data.frame(sim.id = 1:design.points,
                                rlhs,
                                lhs.df,
...
for (sim.id in inANDout.6000.df$sim.id){
  ...
  inANDout.6000.df[sim.id, 14:22] <- as.numeric(out.test)
...
# Running the simpact4emulation function with error catcher in a loop
save(inANDout.6000.df, agedist.data.frame, testinput, iv, file = "/Users/delvaw/Documents/RSimpactHelper/R/Misc/incidence-degree-exploration.6000.RData")
...
# Running the simpact4emulation function with error catcher in a loop
incidence.degree.exploration.6000.loaded <- load(file = "/Users/delvaw/Documents/RSimpactHelper/R/Misc/incidence-degree-exploration.6000.RData")
#save(inANDout.df, file = "/Users/delvaw/Documents/RSimpactHelper/R/Misc/incidence-degree-exploration.100.RData")
#incidence.degree.exploration.6000.loaded
# How many runs gave non-NA output?
nrow(inANDout.6000.df[complete.cases(inANDout.6000.df), ]) # 258

#pairs(inANDout.100.df[, 14:22])
#pairs(inANDout.100.df[, 21:22])
```
...
matched.df <- dplyr::filter(inANDout.6000.df,
                            growth.rate >= 0,
                            growth.rate <= 0.025,
                            prev.men.15.25 >= 0.05,
                            prev.men.15.25 <= 0.15,
...
nrow(matched.df)
names(matched.df)
matchedplusHarling.df <- rbind(matched.df[, 21:22],
                               c(0.075, 0.013))
...
pairs(matchedplusHarling.df[, 1:2],
      ...
      # xlim = c(0, 1),
      # ylim = c(0, 1))
pairs(matched.df[, 8:22])
#pairs(inANDout.100.df[, 8:13])

```


incidence.degree.exploration.6000.loaded <- load(file = "/Users/delvaw/Documents/RSimpactHelper/R/Misc/incidence-degree-exploration.6000.RData")
nrow(inANDout.6000.df[complete.cases(inANDout.6000.df), ]) # 2101
matched.df <- dplyr::filter(inANDout.6000.df,
                            growth.rate >= 0,
                            growth.rate <= 0.04,
                            prev.men.25.50 <= 0.5,
