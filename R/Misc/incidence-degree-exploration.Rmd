...
RS.pc.opt.c <- optimal_params(RS.pc.expt, option="c")
...
prediction.df <- data.frame(matrix(RS.pc.prediction.opt.a, nrow = n, dimnames = list(rownames = 1:n, colnames = names(z.pc.df)[1:4])))
...
sq.array <- as.data.frame(t(apply(prediction.df, 1, function(x) (x - t(targets.pc.vector))^2)))
names(sq.array) <- names(prediction.df)
SumSq <- as.numeric(rowSums(sq.array))
which.min(SumSq)
prediction.df[which.min(SumSq), ] #             PC1        PC2        PC3        PC4
#                                              -0.4337296  1.701795   0.9169298  0.5236572
...
x.estimate <- as.numeric(x.pc.new[which.min(SumSq), ])
...
if (nrow(inANDout.pred.20.df) > 0){
  ...
  ggpairs(inANDout.pred.20.df[, 22:30], colour = "incid.wom.15.30", alpha = 0.4)
}
