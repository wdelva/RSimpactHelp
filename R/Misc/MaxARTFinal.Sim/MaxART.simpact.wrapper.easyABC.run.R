...
all.sim.start <- proc.time()[3]

#source simpact set parameters
inPUT.df.complete <- source("R/Misc/MaxARTFinal.Sim/maxart.simpact.parameters.R")$value
...
source("R/Misc/MaxARTFinal.Sim/maxart.summary.statistics.creator.R")
...
simpact4ABC.chunk.wrapper <- function(simpact.chunk.prior){
  ...
  simpact.chunk.run <- function(input.chunk.params){
    ...
    if(length(chunk.datalist.test)>1){
      ...
      source("R/Misc/MaxARTFinal.Sim/maxart.sim.summary.targets.creator.R")

      ##get the summary statistics as indicated by target.variables
      out.statistic <- max.swazi.sim.summary.creator(chunk.datalist.test)
...
end.chunk.time <- proc.time()[3] - start.chunk.time
