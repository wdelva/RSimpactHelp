...
do
  echo $A
  cat simpact.wrapper.easyABC.run.R | sed  -e  "s/main.filename <- .*/main.filename <- 'INPUT.df-100Points10Par2016-11-03.csv'/" -e  "s/min.chunk <- .*/min.chunk <- ${A}/" -e "s/max.chunk <- .*/max.chunk <- ${B}/" -e "s/sim_repeat <- .*/sim_repeat <- 2/" -e "s/ncluster.use <- .*/ncluster.use <- 16/" > simpact.wrapper.easyABC.run.${A}.R
