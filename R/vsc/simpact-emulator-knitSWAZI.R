...
##We might need to transform any of the summary statistics should the histogram diverge from normality.
par(mfrow=c(1,1))
...
##(We should be able to compare these with the procedural parameters that we would obtain).
z.df <- simpact.z
...
# Use the loop to get iterate through different values. So the optimasation process is faster. Using option B
...
# Use the loop to get iterate through different values. So the optimasation process is faster. Using option C
...
#See the plot of convergency in the B & C matrix of coefficients. -->
ggplot(melt(comp1.B,id.vars=c("run.type")), aes(x=run.type,y=value, color=variable)) + geom_point() + ggtitle("Coeff 1")
ggplot(melt(comp2.B,id.vars=c("run.type")), aes(x=run.type,y=value, color=variable)) + geom_point() + ggtitle("Coeff 2")
...
RS.a.df.check <- data.frame(matrix(RS.opt.a.check, nrow = n.check,
                                   dimnames = list(rownames = 1:n, colnames = paste0("a",names(z.df))[1:length(z.variables)])))
...
RS.b.df.check <- data.frame(matrix(RS.opt.b.check, nrow = n.check,
                                     dimnames = list(rownames = 1:n, colnames = paste0("b",names(z.df))[1:length(z.variables)])))
...
RS.c.df.check <- data.frame(matrix(RS.opt.c.check, nrow = n.check,
                                   dimnames = list(rownames = 1:n, colnames = paste0("c",names(z.df))[1:length(z.variables)])))
...
############ Using the Emulator to Explore the Parameter Space for the None PCA Part to get the interpolated statistics ######
n<-2000
set.seed(1)
...
par(mfrow=c(3,length(z.variables))) # distribution of z.variable i with opt.a then with opt.b
