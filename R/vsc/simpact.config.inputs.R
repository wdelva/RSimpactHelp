...
inANDout.df <- cbind.data.frame(sim.id = 1:design.points, rlhs, lhs.df)

##save(inANDout.df, file = paste0(dirname,"inputANDoutput-",design.points,"Points",variables,"Par",Sys.Date(),"-RData"))

write.csv(inANDout.df, file =paste0("INPUT.df","-",design.points,"Points",variables,"Par",Sys.Date(), ".csv"), row.names = FALSE)

# to break the file and send the file to the bash processing
system("ls -F")
