...
inPUT.df.complete <- simpact.config.inputs(design.points = 100,
...
if(max.chunk > length(inPUT.df.complete)){max.chunk <- length(inPUT.df.complete)}
...
simpact4ABC.chunk.wrapper <- function(simpact.chunk.prior){
  ...
  simpact.chunk.run <- function(input.chunk.params){
    ...
    if(length(chunk.datalist.test)>1){
      ...
      out.statistics <- c(pop.growth.calculator(datalist = chunk.datalist.test),
          prevalence.calculator(datalist = chunk.datalist.test, agegroup = c(15, 50), timepoint = 34)$pointprevalence[3])
      ##get the summary statistics as indicated by target.variables
      out.statistic.no.degree <- out.statistics
