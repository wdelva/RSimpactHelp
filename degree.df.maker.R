...
#'  @examples
#'  load(dataframe)
#'  degree.df <- degree.df.maker(df = dataframe, survey.time = 10, window.width = 1, only.new = TRUE)
#'  @importFrom magrittr %>%
#'  @import dplyr

degree.df.maker <- function(df,
                            survey.time,
                            window.width,
                            only.new = TRUE){
...
# newly formed relationships "else" ongoing relationships.
  {if(only.new)
  {df<-filter(df, FormTime >=survey.time-window.width, DisTime >= survey.time)}
    else
    {df<-filter(df, FormTime < survey.time-window.width, DisTime >= survey.time)}
  }
